...
function getFullUrlFromPath(path) {
  let domain = "https://nothingrandom.studio";
  return domain + path;
}
...
function getScreenshotUrl(fullUrl, options = {}) {
  ...
  const screenshot = `https://v1.screenshot.11ty.dev/${encodeURIComponent(fullUrl)}/opengraph/${getWeeklyServiceCacheBuster()}${o.join("")}/`;
  return screenshot;
}
